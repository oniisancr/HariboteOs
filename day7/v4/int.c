...
// PS/2キーボードからの割り込み  keyboard
void inthandler21(int *esp)
{
    unsigned char data;
    io_out8(PIC0_OCW2, 0x61);   // 通知PIC“IRQ-01”已经处理完了
    data = io_in8(PORT_KEYDAT);	//PORT_KEYDAT=0x0060  按键编码放在这个地址里面
    if(keybuf.len<32){
    	keybuf.data[keybuf.next_w]=data;  //能不断的读取键盘码data
	keybuf.len++;
	keybuf.next_w++;
	if(keybuf.next_w==32){
	    keybuf.next_w=0;
